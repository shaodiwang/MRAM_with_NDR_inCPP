...
class Mythread : public Thread { // Derived from Thread class
    public:
    Mythread( double _Vmtj, double _t_pulse, int _initialstate, int _n_trials, int _this_id, int stack_size, bool joinable, char* _file1, char* _file2, int _write_or_read ) : 
    			Thread(Thread::Options().set_stack_size(stack_size).set_joinable(joinable)), 
    			base_Vmtj(_Vmtj), t_pulse(_t_pulse), initialstate(_initialstate), n_trials(_n_trials), this_id(_this_id), file1(_file1), file2(_file2), write_or_read(_write_or_read){
...
/*    	for (int i=0; i< int(t_step/t_sim); i++){
    ...
    int initialstate,n_trials,this_id,sum,write_or_read;
    char* file1;
    char* file2;
};
