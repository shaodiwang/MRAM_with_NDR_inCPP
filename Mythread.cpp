...
#ifdef OUTNDR
std::fstream foutndr ("ndr.log", std::fstream::out);
std::fstream foutndr2 ("ndr_curve.log", std::fstream::out);
#endif
#ifdef OUTCURVE
...
double tfl = 1.19e-9;                               // Free layer thickness [m]
double Rp = 2e3;                                 // Parallel resistance [Ohms]
...
double Cbline = 25e-15;
double t_delay = 0e-9;                             // Time to initiate pulse application [s]
double t_rise = 0.2e-9;//Rise time
if(write_or_read==3){
    t_rise = Cbline / 20e-15 * 1e-9;
}
if(write_or_read==1){
    t_rise = 0; // ndr write uses precharge operation, which do not allow rise time, while other operations must have rise time to enable equation convergence;
}
double t_sense = 0e-9;
if(write_or_read>=2 ){
    t_sense = 1e-9; // Sensing time for obtaining a voltage margin
}
double t_sim = t_delay + t_pulse + t_rise + t_sense;  //Simulation time
/* -------------------------------------------
 Constants
 -------------------------------------------*/
...
double Hkeff = 1* 2*Ki/(tfl*Ms)/(u0) - Ms*(Nz - max(Nx,Ny));
double Hkeff1 = 1 * 2*Ki1/(tfl*Ms1)/(u0) - Ms1*(Nz - max(Nx,Ny));
...
double** VIGndr =  Read_voltage_current(file1);
double** VIGmos = Read_voltage_current(file2);
double Rmtj = Rap;
if(initial_state != 1){ 
    Rmtj  = Rp;                                    // MTJ resistance [Ohms]
}
double ori_vdd = base_Vmtj + (this_id)*1e-10;
double ori_Vndr = Solve_stable_vndr(VIGndr,VIGmos,Rmtj,ori_vdd);
cout<<"initial Vndr: "<<ori_Vndr<<endl;
...
if(write_or_read == 1){
    vdd = ori_vdd;
    Vndr = ori_Vndr;
    Imtj = IG_V(Vndr,VIGndr,1); //current through MTJ and nmos
    Vmos = V_I(Imtj, VIGmos);
    Indr = Imtj; //current through NDR
}
double d_Rmtj = 0; // delta Rmtj
double d_Imtj = 0; // delta Imtj
double Rmos = 0; // dVm/dImtj
double d_Vndr = 0;
double Csline = 17e-15; // The Cap of source-line (mosfet side)
double d_vdd = 0, new_vdd = 0;
V = Imtj * R; // mtj voltage
...
for(int i=1;i<=int(t_sim/t_step);i++){
    ...
    if ( (t_current>=t_delay + t_rise) && (t_current<=(t_delay+t_pulse+t_rise))){
        new_vdd = ori_vdd;
    }
    else if(t_current > t_delay && t_current < t_delay+t_rise){
        new_vdd = ori_vdd* (t_current - t_delay)/t_rise;
    }
    else{
        new_vdd = 0;
    }
    ...
    vdd = new_vdd;
#ifdef OUTNDR
    foutndr<<"time/vdd/Vndr/Vmtj/Vmos/Imtj/Indr/Rmtj\t"<<i*t_step<<"\t"<<vdd<<"\t"<<Vndr<<"\t"<<V << "\t" << Vmos << "\t"<<Imtj<< "\t" << Indr<<"\t"<<R<<std::endl;
    foutndr2<<i*t_step<<"\t"<<V<<"\t"<<Vndr << "\t" << Vmos << "\t"<<Imtj<< "\t" << Indr<<"\t"<<R<<std::endl;
#endif
    if(write_or_read==3){ // ndr read mode
        if(t_current < t_rise+t_delay+t_pulse){ // precharging
    ...
    else if(write_or_read==2 || write_or_read == 0){ // read or write mode without ndr
        if(t_current < t_rise+t_delay+t_pulse){ // precharging
    ...
    else if (write_or_read == 1){ // NDR write
        d_Imtj = ( t_step*Indr - t_step*Imtj - Cbline*Imtj*d_Rmtj) / ( Cbline/IG_V(Vmos,VIGmos,2) + Cbline*R);
        Imtj += d_Imtj;
        Vmos = V_I(Imtj, VIGmos);
    ...
    //Solve the series of one MTJ and one MOS with cap at MTJ
        if(t_current < t_rise+t_delay+t_pulse){ // precharging
            ...
            Vmos = vdd -V;
            d_Imtj = IG_V(Vmos, VIGmos,1) - Imtj;
            Imtj += d_Imtj;
            V = Imtj*R;
...
//    STT = gammap*J* hbar*(g_tunnel+g_sv)/(2*Ms*tfl*q*u0);
    STT = 0.66*gammap*J* hbar/(2*Ms*tfl*q*u0);
#ifdef DEBUG
    Energy += V/R * V *t_step;
