...
int main(int    argc, char* argv[])
{
	...
	if(argc < 9) {
		cout<<" arguments: [number of trials] [number of threads] [pulse voltage e.g. 0.9V] [pulse width in ns eg 1] [initial_stage 1 or 0]i [VI_ndr] [VI_mos] [0:normal write, 1:NDR write, 2:normal read, 3: NDR read]"<<endl;
		return 1;
	}
...
//			generator=gen_normal(generator, randomArray, int(t_sim/t_step) );
			Mythread* t_mythread = new Mythread( voltage, pulse_t, initial_state, int(trials/n_threads),inner_l, 1024*1024, true,argv[6],argv[7],write_or_read);
